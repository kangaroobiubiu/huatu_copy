...
(function () {
    ...
    function trimText(el) { return el ? el.innerText.trim() : ''; }
    ...
    function parseQuestion(node) {
        ...
        const index = node.getAttribute('data-question-index') || node.getAttribute('data-index') || (node.dataset && node.dataset.index) || '';
        ...
        const choices = choiceEls.length ? choiceEls.map((c, i) => `${String.fromCharCode(65 + i)}. ${c.innerText.trim()}`) : [];
        const jiexiItems = $qa('.jiexi-items .jiexi-item', node).length ? $qa('.jiexi-items .jiexi-item', node) : $qa('.jiexi-item', node);
    ...
    <div class="ht-panel-body">
      ...
      <div class="ht-foot">提示：逐题复制可点击单题右侧按钮。复制后直接粘贴到文档。</div>
    </div>
  `;
    ...
    function renderQuestionList() {
        ...
        qs.forEach(q => {
            ...
            row.innerHTML = `
        <div class="ht-q-left"><label><input type="checkbox" class="ht-q-select"> ${q.index}、 ${q.stem ? (q.stem.length>120? q.stem.slice(0,120)+'...': q.stem) : '(无题干)'}</label></div>
      ...
      `;
            ...
            row.querySelector('.ht-copy-this').addEventListener('click', async () => {
                ...
                try {
                    await safeCopy(text);
                    flashNotice('已复制此题到剪贴板');
                } catch (err) {
                    flashNotice('复制失败：' + (err && err.message ? err.message : err));
                }
            });
        });
        // 添加复制所选按钮（仅一次）
        if (!document.getElementById('ht-copy-selected')) {
            ...
            selBtn.addEventListener('click', async () => {
                ...
                const chosen = rows.filter(r => r.querySelector('.ht-q-select').checked).map(r => r.__q).filter(Boolean);
